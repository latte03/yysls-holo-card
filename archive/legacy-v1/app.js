...
// v1 is an abandoned draft: the root route always hands off to v2. Keep ?v1=1
// as the escape hatch when the old AgX-calibrated route is needed for comparison.
const KEEP_V1=new URLSearchParams(location.search).get('v1')==='1';
...
void main(){
 ...
 // Cycles renders the back plane ~0.3x linear vs the raw texture; match the hero still.
 bg*=.50;
 float w=wave(uv); vec3 foil=spectrum(w*.8+noise(uv*5.)*.12);
 vec3 subject=mix(sub.rgb,overlay(sub.rgb,foil),uFoil*.16);
 bg=mix(bg,overlay(bg,foil),uFoil*.20);
 vec3 col=mix(bg,subject,sub.a);
 float sweep=pow(max(0.,sin((uv.x*.83+uv.y*.35+uView.x*1.8+uView.y*.9)*6.283)),12.);
 col+=foil*sweep*uFoil*.16;
 float line=1.-smoothstep(.06,.25,texture2D(tLine,clamp(su,0.,1.)).r);
 col+=vec3(1.,.94,.78)*line*inside(su)*sub.a*sweep*uFoil*.35;
 col+=vec3(.66,.86,1.)*star(bu)*uFoil*.45*(1.-sub.a*.7);
 vec4 text=texture2D(tText,uv);vec3 tc=mix(text.rgb,overlay(text.rgb,foil),uFoil*.45);tc+=foil*sweep*uFoil*.35*text.a;tc*=.73;col=mix(col,tc,text.a); // .73: Blender renders the text plane at emission 0.5
 ...
 {vec3 _c=gl_FragColor.rgb;vec3 _s=min(_c,vec3(1.));gl_FragColor.rgb=_c-_s+mix(_s,_s*_s*(3.0-2.0*_s),.55);} // linear S-curve (AgX MHC look); >1 headroom kept for bloom, smoothstep inverts above 1.5
 {float _l=dot(gl_FragColor.rgb,vec3(.2126,.7152,.0722));gl_FragColor.rgb=max(mix(vec3(_l),gl_FragColor.rgb,1.25),vec3(0.));} // luma-preserving saturation gain; AgX desaturates more than Cycles' emission-heavy render
}`;
...
async function backArt(){if(!config.assets?.back)return backTexture();const t=await new THREE.TextureLoader().loadAsync(config.assets.back);t.colorSpace=THREE.NoColorSpace;return t;}
async function init(){
 config=await fetch('./card-config.json').then(r=>{if(!r.ok)throw Error('找不到卡牌配置');return r.json();});
 document.title=[config.title,config.collection].filter(Boolean).join(' · ');for(const [id,key]of Object.entries({'card-title':'title','collection':'collection','subtitle':'subtitle','description':'description','tagline':'tagline','technique':'technique','edition':'edition'}))if(config[key])$(id).textContent=config[key];
 renderer=new THREE.WebGLRenderer({antialias:true,alpha:false,preserveDrawingBuffer:true,powerPreference:'high-performance'});renderer.setClearColor(0xffffff,1);renderer.setPixelRatio(Math.min(devicePixelRatio,1.75));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.AgXToneMapping;renderer.toneMappingExposure=1.0;stage.append(renderer.domElement);
 composer=new EffectComposer(renderer);composer.addPass(new RenderPass(scene,camera));composer.addPass(new UnrealBloomPass(new THREE.Vector2(720,1000),.30,.35,1.0));composer.addPass(new OutputPass());
 ...
 const prm=config.parameters||{};uniforms={tSubject:{value:textures[0]},tBackground:{value:textures[1]},tText:{value:textures[2]},tLine:{value:textures[3]},tBack:{value:await backArt()},uTime:{value:0},uView:{value:new THREE.Vector3(0,0,1)},uFoil:{value:prm.foil??.65},uScale:{value:prm.subjectScale??1.25},uDepth:{value:prm.subjectDepth??.4},uBgDepth:{value:prm.backgroundDepth??-.25},uSafeScale:{value:config.safeArea?.scale??1.12},uSafeOffset:{value:new THREE.Vector2(...(config.safeArea?.offset??[-.06,-.085]))}};
...
if(!KEEP_V1){location.replace('../../site/cards/001/');}else init().catch(error=>{console.error(error);loading.textContent='卡牌暂时无法加载。\n'+error.message+'\n请通过本地服务打开网页，并确认素材已生成。';loading.setAttribute('role','alert');window.__holo={ready:false,error:error.message};});
