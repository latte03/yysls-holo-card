...
# Blender 5.1 stopped auto-creating the Background node, so look it up by type
# (the old name lookup raises KeyError there) and create it when still missing.
bg_node=next((n for n in world.node_tree.nodes if n.type=='BACKGROUND'),None) or world.node_tree.nodes.new('ShaderNodeBackground')
bg_node.inputs['Color'].default_value=(.045,.065,.10,1)
bg_node.inputs['Strength'].default_value=.4
