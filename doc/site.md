...
卡壳 GLB 八张卡逐字节相同、六个材质共享，所以换卡只需要换贴图——`viewer/app.js` 拦下卡序链接的
...
## 主体构图旋钮：subjectScale 是反的

着色器 `fitUV()`（`viewer/app.js:84`）是 **卡面 → 纹理** 的映射：`su = (uv-.5)*S + .5 + offset`。
所以 `parameters.subjectScale` **小于 1 才是放大**（S=0.76 = 把纹理放大 1.32 倍贴进卡面），
大于 1 是缩小并把纹理外缘推进画面。`inside(su)` 又把卡外那部分 alpha 直接归零，所以放大后
超出卡边的内容是被卡的边裁掉的（等价 `overflow:hidden`），不会拉伸出 smear。

`subjectLayers.mid.offset` 是卡面 uv 位移，**x 正值往左、y 正值往下**（0.01 = 卡宽 1%）。
单层卡也能写 `mid`（只写调参不给 `src`），这样卡片页面板的「大小 / 左右 / 上下」滑杆才接得上。

**给新素材定放大倍率别用固定行的宽度比**：发丝蓬松度、手的位置都会污染它。可靠做法是把两版
主体的 alpha 掩膜在卡空间里做 IoU 扫描（`S` 和 `offset_v` 二维网格，取 IoU 峰值），009 换宽袖
版时手工量出行宽比 1.30、IoU 拟合也是 1.32（S=0.76），而"脸组件包围盒"给出 1.03 是错的
（把手一起框进去了）。
